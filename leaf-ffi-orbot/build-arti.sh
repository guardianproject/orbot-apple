...
WORKDIR="$SCRIPTDIR/../arti-orbot-ios"

FILENAME="libartiorbotios.a"
...
if [[ -n "${DEVELOPER_SDK_DIR:-}" ]]; then
	...
	cargo lipo $RELEASE --targets $TARGETS --manifest-path "${WORKDIR}/Cargo.toml" -p arti-orbot-ios
	...
	cargo lipo --targets $TARGETS --manifest-path "${WORKDIR}/Cargo.toml" -p arti-orbot-ios -v
fi

cbindgen --config "${WORKDIR}/cbindgen.toml" "${WORKDIR}/src/lib.rs" > "${SCRIPTDIR}/arti.h"
